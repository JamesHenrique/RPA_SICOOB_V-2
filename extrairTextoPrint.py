...
def qntComprovantePix(cliente):
    ...
    imagem = cv2.resize(imagem, None, fx=1.5, fy=1.5, interpolation=cv2.INTER_CUBIC)  # Redimensiona a imagem
    ...
    # Usando expressão regular para encontrar o número após "Total de registros: "
    match = re.search(r"Total de registros.\s*(\d+)", texto)
    numero = ''
    
  
    try:
        if match:
            numero = int(match.group(1))  # Captura o número corretamente
            atualizar_total_cp(cliente, numero)
            print(numero)
        else:
            atualizar_total_cp(cliente, 0)  # Se não encontrar, assume 0
    except:
